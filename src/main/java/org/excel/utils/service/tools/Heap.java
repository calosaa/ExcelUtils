package org.excel.utils.service.tools;
...
public class Heap {
     ...
     * 堆排序
     */
    public static void strHeapSort(String[] arr){
        //为什么从arr.length/2-1开始？
        for (int i = arr.length/2-1; i >= 0 ; i--) {
            strAdjustHeap(arr,i,arr.length);
        }
     ...
     * @param length 进行调整的元素的个数，length是在逐渐减少的
     */
    public static void strAdjustHeap (String[] arr,int i,int length){
        /*取出当前非叶子结点的值保到临时变量中*/
        String temp = arr[i];

        /*j=i*2+1表示的是i结点的左子结点*/
        for (int j = i * 2 + 1; j < length ; j = j * 2 + 1) {
            if (j+1 < length && arr[j].compareTo(arr[j+1])<0){ //左子结点小于右子结点
                j++; //j指向右子结点
            }
            if (arr[j].compareTo(temp)>0){ //子节点大于父节点
                arr[i] = arr[j]; //把较大的值赋值给父节点
                //arr[j] = temp; 这里没必要换
                i = j; //让i指向与其换位的子结点 因为
            }else{
